...
<?php
	...
	$db->select('tb_profil','tb_profil.*, tb_profil.uid as profId, tb_karyawan.*','tb_karyawan ON tb_karyawan.profil_id = tb_profil.uid', 'tb_karyawan.uid='.$uid, null); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
	$res = $db->getResult();
...
<form name="formName" action="main.php?module=karyawan&process=update" method="post" role="form" onsubmit="return validate();">
	...
	<div class="form-group">
		...
		<input type="text" name="nama_kar" class="form-control" id="nama_kar" value="<?php echo $data['fullname'] ?>" />
	</div>
